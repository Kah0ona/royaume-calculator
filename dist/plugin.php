...
class CalculatorPlugin {
	protected $to = 'info@rm-facility.com';
	...
	public function process(){
		...
		mail($post['personal']['E-mail'], 'Uw aanvraag', $mail, $headers);
		echo '{"status" : "OK"}';
		die();
	}	
	...
	public function render_calculator_package($atts) {
		extract(shortcode_atts(
			array('companymail'=>'info@rm-facility.com'
			), $atts)
		);
		$this->render_calculator('PACKAGE', $companymail);
	}
	public function render_calculator_hourly($atts){
		extract(shortcode_atts(
			array('companymail'=>'info@rm-facility.com'
			), $atts)
		);
		$this->render_calculator('HOURLY', $companymail);
	}
	public function render_calculator($type='PACKAGE', $email){ ?>
		<script>
			jQuery(document).ready(function($){
				window.app.model.init({
					submitUrl : "<?php echo admin_url('admin-ajax.php'); ?>",
					companymail : "<?php echo $email ?>",
